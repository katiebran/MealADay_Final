...
var _layeredCache = require('layered-cache');

var _layeredCache2 = _interopRequireDefault(_layeredCache);

var _compare = require('./compare');

var _utils = require('./utils');

var _lodash = require('lodash.findlastindex');
...
class DataSourceSpan {
  constructor(span, {
    ...
    const db = this._db = new _db2.default({
      client,
      connection,
      code,
      span,
    ...
    const layers = [new Filter(isTrading), lru, db];

    if (!offline) {
      layers.push(loader);
    }

    this._source = new _layeredCache2.default(layers, {
      isNotFound
    });
  ...
  _isClosed(time) {
    return (0, _compare.isClosed)(time, this._span);
  }

  async _update(data, { requestTime } = {}) {
    const index = (0, _lodash2.default)(data, ({ time }) => this._isClosed(time));
    const closedDataPairs = !!~index ? data.slice(0, index + 1).map(value => [value.time, value]) : [];
  ...
  _closestClosedTime(time) {
    const closedTimestamp = this._isClosed(time) ? (0, _utils.Time)(time, this._span).timestamp()
    // If the request time is not closed,
    // then use the previous time of the span
    : (0, _utils.Time)(time, this._span).prev();
  ...
  async lastUpdated() {
    return this._lastUpdated || (this._lastUpdated = await this._db.lastUpdated());
  }
  ...
  get(...times) {
    ...
    return length === 0 ? [] : length === 1 ? this._source.get(times[0]) : this._source.mget(...times);
  }
